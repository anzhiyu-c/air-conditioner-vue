 ...
 * @LastEditTime: 2022-11-29 21:02:49
 * @LastEditors: 安知鱼
 */
...
const ripple = (event: PointerEvent, el: HTMLElement, options: IRippleDirectiveOptions): void => {
  ...
  const size = MULTIPLE_NUMBER * getDistanceToFurthestCorner(x as any, y as any, rect);

  const rippleContainer = createContainer(computedStyles);
  const rippleEl = createrippleElement(x as any, y as any, size, options);
